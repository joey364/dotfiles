...
function addDependencyWithType(dependencyType = "dependencies") {
    return __awaiter(this, void 0, void 0, function* () {
        const api = new pubApi_1.PubAPI();
        const isPubspecExists = fs.existsSync(pubspecFile);
        if (!isPubspecExists) {
            messaging_1.showError(new pubError_1.PubError("Pubspec file not found on workspace root."));
            return;
        }
...
exports.addDependencyWithType = addDependencyWithType;
function getPubspecText() {
...
function addDependencyByText(pubspecString, newPackage, dependencyType = "dependencies") {
    let insertionMethod = InsertionMethod.ADD;
    let lines = pubspecString.split("\n");
    let dependencyTypeQuery = `${dependencyType}:`;
    let dependencyLineIndex = lines.findIndex((line) => line.trim() === dependencyTypeQuery);
...
exports.default = addDependencyWithType;
